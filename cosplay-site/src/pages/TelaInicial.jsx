...
export default function TelaInicial() {
  ...
  return (
    <Layout title="Cosplays disponíveis" backTo="/" showSilhouette={false}>
      <ul className="nav nav-tabs mb-4">
        <li className="nav-item w-50">
          <button 
            className={`nav-link w-100 ${activeTab === 'perfil' ? 'active' : ''}`} 
        ...
        <li className="nav-item w-50">
          <button 
            className={`nav-link w-100 ${activeTab === 'galeria' ? 'active' : ''}`} 
      ...
      <div className="tab-content">
        <div className={`tab-pane fade ${activeTab === 'perfil' ? 'show active' : ''}`}>
          ...
          <p className="small text-muted">Gerencie seu perfil e veja seus cosplays favoritos.</p>

          <button className="btn btn-primary w-100" onClick={handleEditarPerfil}>
            Editar Perfil
          </button>
        </div>

        <div className={`tab-pane fade ${activeTab === 'galeria' ? 'show active' : ''}`}>
          {cosplays.length > 0 ? (
            ...
            <div className="text-center py-5">
              <p className="text-muted mb-3">Você ainda não tem cosplays cadastrados</p>
              <button className="btn btn-primary" onClick={handleAdicionarCosplay}>
                Adicionar Cosplay
              </button>
            </div>
          )}
